...
_TEMPLATE_PATH = Path(__file__).resolve().parent.parent / "templates" / "report_googlemap.html"
...
def _render_map_section(
    ...
    if not google_maps_api_key:
        ...
        return (
            ...
            "Set GOOGLE_MAPS_API_KEY to render the interactive Google map in this report."
            "</div>"
            '<div class="grid grid-cols-1 sm:grid-cols-2 gap-2">'
            f'<a class="block rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm font-semibold text-slate-700 hover:bg-slate-100" href="{search_hospitals}" target="_blank" rel="noopener">🏥 Search nearby hospitals</a>'
            f'<a class="block rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm font-semibold text-slate-700 hover:bg-slate-100" href="{search_parks}" target="_blank" rel="noopener">🌳 Search nearby parks</a>'
            "</div>"
            "</div>"
        )
...
(function() {
  ...
  function doMapSearch(mode) {
    ...
    var request = {
      location: userPoint,
      radius: 5000,
      type: mode === 'hospital' ? 'hospital' : 'park'
    };
    placesService.nearbySearch(request, function(results, status) {
      ...
      results.slice(0, 6).forEach(function(place, idx) {
        ...
        var dist = google.maps.geometry.spherical.computeDistanceBetween(
          new google.maps.LatLng(userPoint.lat, userPoint.lng),
          point
        );
        ...
        card.innerHTML = '<div class="map-place-icon '+mode+'">'+(mode==='hospital'?'\ud83c\udfe5':'\ud83c\udf33')+'</div>'
          +'<div style="flex:1;min-width:0"><div style="font-size:0.9em;font-weight:600;color:#222;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+escapeHtml(place.name || 'Unknown place')+'</div>'
  ...
  function focusMapCard(idx, place) {
    ...
    directionsRenderer.setOptions({
      polylineOptions: { strokeColor: routeColor, strokeOpacity: 0.85, strokeWeight: 5 }
    });
  ...
  function navigateMapTo(place) {
    ...
    var url = 'https://www.google.com/maps/dir/?api=1'
      + '&origin=' + encodeURIComponent(userPoint.lat + ',' + userPoint.lng)
      + '&destination=' + encodeURIComponent(dest.lat() + ',' + dest.lng())
      + '&destination_place_id=' + encodeURIComponent(place.place_id || '')
      + '&travelmode=walking';
  ...
  window.addEventListener('error', function(event) {
    if (String(event.message || '').toLowerCase().indexOf('google') >= 0) {
      var container = document.getElementById('map-cards');
      if (container) {
        container.innerHTML = '<div style="text-align:center;color:#b45309;padding:16px;font-size:0.85em">Google Maps failed to load. Check GOOGLE_MAPS_API_KEY and enabled APIs.</div>';
      }
    }
  });

  if (!window.google || !google.maps) {
    window.initPatientReportMap = window.initPatientReportMap || function() {
      var container = document.getElementById('map-cards');
      if (container) {
        container.innerHTML = '<div style="text-align:center;color:#b45309;padding:16px;font-size:0.85em">Google Maps failed to load. Check GOOGLE_MAPS_API_KEY and enabled APIs.</div>';
      }
    ...
    js = js.replace("__FALLBACK_LON__", lon_js)
    js = js.replace("__FALLBACK_LAT__", lat_js)
