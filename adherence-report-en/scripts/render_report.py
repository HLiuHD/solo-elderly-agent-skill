...
Renders a patient-facing adherence HTML report (English).
...
_FIELD_LABELS = {
    ...
    "cause_if_known": "Possible reason",
...
def _render_personalized_context(so: dict, payload: dict, memory: dict) -> str:
    ...
    if evidence_items:
        ...
        for item in evidence_items[:4]:
            ...
            icon, label = category_meta.get(category, ("🎯", "Personal context"))
            ...
            if why_it_matters:
                body += (
                    ...
                    f'Why it matters: {escape(why_it_matters)}</div>'
                )
            if body:
                cards.append(
    ...
    if history_chips:
        ...
        if any(c in {"高血压", "Hypertension"} for c in conditions):
            implication_parts.append("keep sodium lower")
        if any(c in {"2型糖尿病", "糖尿病", "Type 2 diabetes", "Diabetes"} for c in conditions):
            implication_parts.append("keep meals regular and reduce refined sugar")
        ...
        cards.append(
            ...
            '<div class="flex-1 min-w-0">'
            '<div class="sub-card-label">Health history</div>'
            '<div class="sub-card-value">These suggestions reflect your conditions and recovery stage</div>'
            '</div>'
            '</div>'
            ...
            + (
                ...
                f'What this changes: {escape("; ".join(implication_parts))}</div>'
    ...
    if med_issue:
        med_body = ""
        if med_issue:
            med_body += f'<div class="text-sm text-slate-700 leading-relaxed">Recent note: {escape(med_issue)}</div>'
        if med_adjustment:
            med_body += (
                '<div class="mt-3 text-xs text-slate-600 bg-slate-50 rounded-xl px-3 py-2 leading-relaxed border border-slate-200">'
                f'What to focus on: {escape(med_adjustment)}</div>'
            )
        elif "恶心" in med_issue or "nausea" in med_issue.lower():
            med_body += (
                '<div class="mt-3 text-xs text-slate-600 bg-slate-50 rounded-xl px-3 py-2 leading-relaxed border border-slate-200">'
                'What to focus on: smaller meals, warm soft foods, and discussing ongoing discomfort with your clinician.</div>'
            )
        ...
        cards.append(
            ...
            '<span class="sub-card-icon">💊</span>'
            '<div class="flex-1 min-w-0">'
            '<div class="sub-card-label">Current medication</div>'
            '<div class="sub-card-value">Medication routines and side effects shape the nutrition plan</div>'
            '</div>'
            '</div>'
    ...
    if monitoring_gap:
        recent_focus.append("Monitoring note: " + monitoring_gap)

    if recent_focus:
        implication = []
        if any("Glucose" in bit for bit in metric_bits):
            implication.append("keep meals regular")
        if any("Steps" in bit for bit in metric_bits):
            implication.append("build activity gradually")
        if monitoring_gap:
            implication.append("close the monitoring gap")
        ...
        cards.append(
            ...
            '<span class="sub-card-icon">📈</span>'
            '<div class="flex-1 min-w-0">'
            '<div class="sub-card-label">Recent readings</div>'
            '<div class="sub-card-value">Advice grounded in your recent records</div>'
            '</div>'
            '</div>'
            '<div class="sub-card-body">'
            ...
            + (
                '<div class="text-xs text-slate-600 bg-slate-50 rounded-xl px-3 py-2 leading-relaxed border border-slate-200">'
                f'What this changes: {escape("; ".join(implication))}</div>'
                if implication else ""
            )
            + '</div></div>'
        )
...
def _render_diet_table(diet_table: list[dict]) -> str:
    ...
    return (
        ...
        '<th class="px-4 py-2 text-left text-xs font-semibold text-slate-500">Choose</th>'
...
def _render_map_section(
    ...
    html = (
        ...
        '<div class="journey-section-title">Nearby support</div>'
        '<div class="journey-section-copy">Find hospitals and parks near your current location.</div>'
        ...
        '<div id="bmap"></div>'
        '<button id="locate-btn" title="Back to my location">📍</button>'
        "</div>"
...
def _metric_delta(first: float, last: float, unit: str = "") -> tuple[str, str]:
    ...
    return f"From start {arrow} {_format_number(abs(diff))}{unit}", direction
...
def _build_trend_data(payload: dict) -> dict:
    ...
    metric_meta = {
        "blood_pressure": ("🩺", "Blood pressure", "mmHg", "#7c3aed", "The curve shows systolic pressure; tap a point for the full reading."),
        "heart_rate": ("🫀", "Heart rate", "bpm", "#ef4444", "See how your heart rate has changed."),
        "blood_oxygen": ("🫁", "Blood oxygen", "%", "#06b6d4", "See how your blood oxygen has changed."),
        "steps_today": ("👟", "Steps", "steps", "#2563eb", "See how your activity has changed."),
        "steps": ("👟", "Steps", "steps", "#2563eb", "See how your activity has changed."),
        "blood_glucose": ("🧪", "Glucose", "mmol/L", "#f59e0b", "See how your glucose has changed."),
    }
    ...
    for window_key in ("week", "month", "quarter"):
        ...
        if metric_cards:
            ...
            aggregate_label = "weekly average" if window.get("granularity") == "week" else "daily average"
...
def _render_trend_story(trend_data: dict) -> str:
    if not trend_data:
        return ""
    label_map = {"week": "Week", "month": "Month", "quarter": "Quarter"}
    ...
    return (
        ...
        '<div>'
        '<div class="trend-card-title">Signal trends</div>'
        '<div class="trend-card-copy">Slide along a curve to see daily or weekly averages.</div>'
        '</div>'
...
def _journey_page(page_key: str, title: str, subtitle: str, body: str, active: bool = False) -> str:
    if not body or not body.strip():
        body = (
            ...
            '<div class="journey-section-title">Nothing to show yet</div>'
            '<div class="journey-section-copy">This page will update when new records are available.</div>'
            '</section>'
        )
...
def _adherence_brief(key: str, dim: dict) -> str:
    ...
    return status or "No additional details"
...
def _adherence_chip_label(key: str, dim: dict) -> str:
    ...
    if key == "medication":
        ...
        return status or "Medication to review"
    if key == "appetite":
        if any(word in text.lower() for word in ("salt", "oil", "fried", "hot pot")):
            return "High-salt or oily meals"
        return status or "Diet to review"
    if key == "exercise":
        return "Activity not completed" if _adherence_tone(status) != "good" else "Activity completed"
    if key == "monitoring":
        return "Monitoring gap" if _adherence_tone(status) != "good" else "Monitoring completed"
    return status or "To review"
...
def _render_journey_status(so: dict) -> tuple[str, str, str]:
    ...
    for key, icon, title in _ADHERENCE_DIMENSIONS:
        ...
        if isinstance(dim, dict) and (dim.get("status") or _adherence_brief(key, dim)):
            status = _localize_zh_text(str(dim.get("status") or "To review"))
            tone = _adherence_tone(status)
            dims.append((key, icon, title, dim, status, tone))

    if not dims:
        return "", "To review", "No adherence status is available yet."

    tones = [item[-1] for item in dims]
    if "alert" in tones:
        overall = "Needs attention"
    elif "warn" in tones:
        overall = "Partly completed"
    else:
        overall = "On track"
    ...
    body = (
        '<section class="journey-card">'
        '<div class="journey-section-kicker">Today\'s overall status</div>'
        f'<div class="journey-section-title">{escape(overall)}</div>'
        f'<div class="status-alert-row">{"".join(chips)}</div>'
        '</section>'
        '<section class="journey-card">'
        '<div class="journey-section-title">Adherence overview</div>'
...
def _render_journey_vitals(summary: dict) -> str:
    ...
    return (
        '<section class="journey-card">'
        '<div class="journey-section-title">Latest health readings</div>'
...
def _render_journey_home(so: dict, payload: dict, memory: dict, current_time_raw: str) -> str:
    ...
    avatar = escape(name[:1] if name and name != "You" else "Y")
    ...
    hero = (
        ...
        f'<div class="journey-hero-text">Start with today\'s status, then focus on the few steps that matter most.</div>'
        '</div>'
        '</div>'
...
def _render_journey_tasks(so: dict, payload: dict) -> str:
    ...
    for idx, rec in enumerate(recs[:3], start=1):
        ...
        cards.append(
            ...
            + (f'<div class="journey-task-reason">Why: {escape(reason)}</div>' if reason else "")
            + '<div class="journey-task-actions feedback-actions">'
            + f'<button class="feedback-btn like" data-day-idx="-1" data-meal-type="priority" '
            f'data-item-name="{safe_text}" data-default-label="Keep this" data-active-label="Kept" '
            f'onclick="saveLikeFromButton(this)">Keep this</button>'
            + f'<button class="feedback-btn feedback-skip" data-day-idx="-1" data-meal-type="priority" '
            f'data-item-name="{safe_text}" data-default-label="Not for me" data-active-label="Skipped" '
    ...
    return (
        '<section class="journey-card">'
        '<div class="journey-section-title">Today\'s three most useful actions</div>'
        '<div class="journey-section-copy">Focus on a few practical steps for today.</div>'
...
def _render_nutrition_focus_pills(so: dict) -> str:
    ...
    micro_copy = {
        "low_salt": "Less salt",
        "low_oil": "Keep it light",
        "protein": "Support recovery",
        "hydration": "Sip regularly",
        "fiber": "Keep intake steady",
        "meal_rhythm": "Keep a rhythm",
    }
    parts = []
    for item in priorities[:4]:
        ...
        copy = micro_copy.get(category) or "Start today"
        parts.append(
    ...
    return (
        '<section class="journey-card">'
        '<div class="journey-section-title">Today\'s nutrition focus</div>'
...
def _first_meal_items(meal_items: list) -> tuple[str, str]:
    ...
    for meal_key, meal_label in (("dinner", "Dinner"), ("lunch", "Lunch"), ("breakfast", "Breakfast")):
...
def _render_diet_advice_cards(diet_table: list[dict]) -> str:
    ...
    for item in diet_table[:4]:
        ...
        cards.append(
            ...
            f'<div class="diet-advice-title">{escape(condition or "Eating pattern")}</div>'
            ...
            '<div class="diet-advice-label">Choose more often</div>'
            f'<div class="diet-advice-copy">{escape(recommend or "Keep your eating pattern consistent with clinical advice")}</div>'
            '</div>'
            '<div class="diet-advice-column">'
            '<div class="diet-advice-label">Limit or avoid</div>'
            f'<div class="diet-advice-copy">{escape(avoid or "Avoid sudden major diet changes")}</div>'
            '</div>'
            '</div>'
            '</div>'
        )
...
def _render_journey_nutrition(
    ...
    if meal_plan_html:
        plan_html = (
            '<section class="journey-card">'
            '<div class="journey-section-title">Meal plan</div>'
            f'{meal_plan_html}'
            '</section>'
        )
    ...
    diet_html = (
        ...
        '<div class="journey-section-title">Food guidance by condition</div>'
        '<div class="journey-section-copy">Start with the food choices most relevant to your conditions and medications.</div>'
        f'{diet_cards or diet_table}'
        '</section>'
    ) if (diet_cards or diet_table) else ""
    advice_html = (
        '<section class="journey-card">'
        '<div class="journey-section-title">Nutrition notes</div>'
...
def _render_journey_why(so: dict, payload: dict, memory: dict) -> str:
    ...
    if context_html:
        sections.append(
            '<section class="journey-card">'
            '<div class="journey-section-title">Why these suggestions fit</div>'
            '<div class="journey-section-copy">See the medication, diet, activity, and monitoring context separately.</div>'
            f'<div class="why-stack mt-3">{context_html}</div>'
            '</section>'
        )
    note = _build_supportive_note(so, payload)
    sections.append(f'<div class="why-footer-note">Small steps add up. {escape(note)}</div>')
...
def _build_supportive_note(so: dict, payload: dict) -> str:
    ...
    if "decreased" in appetite.lower() or "low appetite" in appetite.lower():
        return (
            "Your appetite has been lower recently, so nutrition deserves extra attention while your body recovers. "
            "You do not need a large meal today; one small protein-rich snack is already a useful start."
        )
    if steps not in (None, ""):
        try:
            if float(_extract_numbers(steps)[0]) < 3000:
                return "The goal today is not to do everything at once. Gently rebuilding activity and nutrition, even with a short walk, still moves recovery forward."
        except (IndexError, ValueError, TypeError):
            pass
    sections = so.get("assistant_message_sections") or []
    if sections:
        return _localize_zh_text(str(sections[0].get("content") or ""))
    return _localize_zh_text(so.get("assistant_message_patient") or "Focus on the few steps that will help most today, one at a time.")
...
def main() -> None:
    ...
    if any(
        ...
        ai_map_msg = "Your recent activity has been low. A gentle walk in a nearby park may help when the weather is comfortable."
        ai_map_msg_park = ai_map_msg
    else:
        ai_map_msg = "Nearby care locations are available if you need them."
        ai_map_msg_park = "A light walk in a nearby park can support mood and mobility."
    ...
    submit_cta_html = (
        ...
        'class="submit-cta-button" disabled>Choose preferences first</button>'
        '<p class="submit-cta-hint" id="submitHint">'
        'Choose a cuisine or mark an item as useful or not for me</p>'
        '</div>'
    ...
    home_page_html = _journey_page(
        "home",
        "Home",
        "Start with today's status, adherence, and latest health readings.",
    ...
    plan_page_html = _journey_page(
        "plan",
        "Today's plan",
        "Focus on these three actions today.",
        plan_page_body,
    )
    nutrition_page_html = _journey_page(
        "nutrition",
        "Nutrition and meals",
        "See today's nutrition priorities and meal suggestions.",
        _render_journey_nutrition(so, meal_items, meal_plan_html, current_time_raw),
    )
    trend_page_html = _journey_page(
        "trends",
        "Health trends",
        "See recent blood pressure, heart rate, and blood oxygen changes.",
        trend_story_html,
    )
    why_page_html = _journey_page(
        "why",
        "Why these suggestions",
        "See how each suggestion connects to your recent situation.",
    ...
    html = template.format(
        ...
        guardrail=escape(
            ...
            or "This report is generated by an AI health assistant for everyday health management. It does not replace medical diagnosis or treatment. Contact a clinician or emergency services if you feel unwell."
        ),
    )
